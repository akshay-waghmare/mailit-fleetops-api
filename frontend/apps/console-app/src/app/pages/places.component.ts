...
import { PlaceDetailModalComponent } from '../components/place-detail-modal.component';
...
export class PlacesComponent implements OnInit, OnDestroy {
  ...
  viewPlace(place: PlaceRecord): void {
    const dialogRef = this.dialog.open(PlaceDetailModalComponent, {
      ...
      maxWidth: '90vw',
      maxHeight: '90vh',
      panelClass: 'place-detail-dialog'
    });
  ...
  closeViewModal(): void {
    // No longer needed with MatDialog, keeping for compatibility
    this.showViewModal = false;
    this.selectedPlaceForView = undefined;
  }
  ...
  duplicatePlace(place: PlaceRecord): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        // Refresh the places list to show the new duplicate
        this.loadPlaces();
      }
    });
  }
